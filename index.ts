...
async function main() {
  ...
  const assetId = BigInt(createResult.confirmation.assetIndex!); //Asset 고유의 ID, 흔히 이더리움에서는 토큰마다 스컨 주소가 있는데 여기는 스컨이 아니기때문에 asset마다 고유의 id가 있음
  ...
  // 문제 4 시작
  await algorand.send.assetOptIn({ sender: alice.addr, assetId: assetId });
  // 문제 4 끝
